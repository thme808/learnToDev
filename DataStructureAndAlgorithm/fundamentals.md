# 자료구조와 알고리즘의 기본 
## 자료구조와 알고리즘 공부의 필요성
알고리즘적 사고는 이 해결책을 다양한 방법으로, 더 간결하게, 혹은 더 우아한 방식으로 제시한다. 데이터 구조와 알고리즘은 프로그래밍 언어에 비종속으로 공부할 수 있다는 장점이 있다. (물론 어떤 프로그래밍 언어에서 지원하는 데이터 구조가 다른 프로그래밍 언어에서 지원하지 않는 경우도 있다) 결국 데이터 구조와 알고리즘을 공부한다는 것은 선대 개발자들의 브레인스토밍 결과를 떠먹고, 내가 가진 현재의 문제 해결에 적용하는 데 의의가 있다.
